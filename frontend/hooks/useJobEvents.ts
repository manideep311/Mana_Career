...
const INITIAL: JobEventState = { status: null, message: null, done: false, error: null };
...
export function useJobEvents(
  ...
  const enabled = (opts.enabled ?? true) && jobId !== null;
  const baseDelayMs = opts.baseDelayMs ?? 1000;
  const [state, setState] = useState<JobEventState>(INITIAL);
  ...
  useEffect(() => {
    ...
    const consume = async (): Promise<"done" | "closed"> => {
      const res = await authedStream(`/api/v1/jobs/${jobId}/events`, {
        signal: ctrl.signal,
      ...
      for (;;) {
        ...
        while ((m = /\r\n\r\n|\n\n/.exec(buf)) !== null) {
          ...
          if (frame.event === "status") {
            setState((s) => ({
              ...s,
              status: (frame.data.status as JobStatus) ?? s.status,
              message: (frame.data.message as string) ?? s.message,
            }));
          } else if (frame.event === "done") {
            setState((s) => ({ ...s, done: true, status: (frame.data.status as JobStatus) ?? s.status }));
            return "done";
  ...
  }, [enabled, jobId, authedStream, baseDelayMs]);
