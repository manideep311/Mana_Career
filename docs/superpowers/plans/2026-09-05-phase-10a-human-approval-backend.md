...
**Status: shipped.** 11 implementation tasks + controller closeout, via subagent-driven-development (fresh Sonnet implementer per task; dedicated subagent reviews for the three highest-risk tasks — the migration, the worker interrupt/resume refactor, and the DB-gated integration test — all PASS; inline controller review for the rest), on branch `phase-10a-human-approval-backend` off `main@7b7b41a`.

**What changed:** LangGraph's `interrupt()`/`Command(resume=...)` mechanism, used for the first time in this codebase, to pause the `prepare_application` agent chain for human approval and resume it in a separate later worker invocation. Concretely: three new agent nodes — `application_prep` (deterministic snapshot + sha256 hash → `Application` + pending `ApprovalRequest`), `human_approval` (**raw**, un-`guard()`-wrapped — `interrupt()` raises `GraphInterrupt`, an `Exception` subclass `guard()`'s bare `except` would otherwise swallow), `email_external_action` (re-asserts `approved` + re-verifies the payload hash against the current rows, sends via a sandboxed `ConsoleEmailSender`, writes an `audit_logs` row). `run_agent`'s astream-drive logic extracted into a shared `_drive()` used by both `run_agent` (fresh init) and a new `resume_agent` worker task (`Command(resume=...)` on the same `thread_id`). Two new tables (`applications`, `approval_requests`, migration `0013`, with a partial-unique index enforcing ≤1 pending approval per run). Two minimal API routers (`POST /applications` + `GET /applications/{id}`; `GET /approvals` + `GET /approvals/{id}` + `POST /approvals/{id}` which flips the row's state machine and calls `AgentService.resume_run`). `guard()` gained a defensive `except GraphBubbleUp: raise`. `ManaState.status` gained `"awaiting_approval"` (the DB CHECK constraint already allowed it, from Phase 7a). 34 files changed, +1514/-101, 12 commits (`0a58700`..`fa4105f`).

**Rulings (all 12 recorded in the spec addendum before any task was dispatched; several backed by direct verification against the installed `langgraph==1.2.11`, including a real running interrupt/resume script):** `human_approval` is raw; `guard()` re-raises `GraphBubbleUp`; the interrupt's re-execution-from-node-start semantic means all writes happen in the once-only `application_prep`, `human_approval` only reads; the interrupt payload is a bare `{"approval_id": ...}` (frontend fetches the full snapshot via `GET /approvals/{id}`); resume needs a second worker task, not a second `run_agent` call; pause vs. finish is detected via `graph.aget_state(...).interrupts`; **no reject→revise loop** (reject is terminal this phase); **no edits/reconfirm flow**; `/applications` is two endpoints not the full resource (Phase 11 owns the tracker); **no `/approvals/{id}/cancel`**; `EmailSender` is console-only.

**Process notes — three real plan gaps caught by implementers/reviewers, all fixed:** (1) Task 7's addition of a required `AgentDeps.email_sender` field broke the *pre-refactor* `run_agent`'s `AgentDeps(...)` call — fixed in Task 7. (2) `nodes/__init__.py`'s re-export shadows `human_approval` as a package attribute, breaking a test's `import ... as mod` — fixed via `importlib.import_module` (controller empirically confirmed the shadowing is real). (3) Phase 9's `test_prepare_application_task.py` still asserted `prepare_application` runs to `"completed"`, which Task 7's wiring broke — reframed in closeout (`fa4105f`) to assert the artifact-assembly + pause, since Task 11's `test_resume_agent.py` now owns the full pause/resume/send flow. Closeout also fixed a glibc-only `strftime('%-I…')` (→ portable `.lstrip("0")`) and dropped an unused `_drive()` param — both surfaced by reviews.

**The load-bearing fact — verified three times independently:** an interrupting node yields `{'__interrupt__': (Interrupt(value=..., id=...),)}` (a *tuple* value → the `isinstance(partial_state, dict)` guard in `_drive`'s astream loop is both correct and necessary); `snap.interrupts` is truthy while paused and `()` once resumed to a terminal state; `Command(resume={...})` on the same `thread_id` re-executes the interrupt node and continues. Confirmed by the controller (while writing the plan), the Task 8 reviewer, and the Task 11 reviewer — each with their own standalone script against the real installed package.

**Regression check:** local gate green throughout — ruff / mypy (`3 kept, 0 broken` import contracts) clean at every task boundary and on final `main`. Backend test count: 393 (baseline at `7b7b41a`, verified directly in an isolated worktree) → 414 (branch tip). mypy source-file count: 148 → 159. `alembic heads`: single head, `0012_application_documents` → `0013_applications_approvals`. All 24 pure tests added/modified across the phase pass on the branch tip; every DB-gated test (`test_application_approval_models.py`, `test_resume_agent.py`, `test_prepare_application_task.py`, `test_applications.py`, `test_approvals.py`) collects error-free locally and is left for CI.

**CI:** pending — watched to green as part of this closeout. This is the first push where the `backend` job's DB-gated suite exercises real interrupt/resume machinery end to end, and the first exercise of the `0013` migration's partial-unique-index DDL against a real Postgres.

**Not verified here (flagged, not addressed):** the full 16-node interrupting chain was reproduced end to end only against a 2-node analogue in the reviewers' scripts, not the real graph (Task 11's DB-gated test is the real proof, and only runs in CI). Cross-*process* resume in production (a `run_agent` worker pauses, a *different* `resume_agent` worker process resumes) relies on the persistent `AsyncPostgresSaver` — unchanged by this phase but a prerequisite that the in-process `MemorySaver` test can't exercise. Everything Phase 10b (the 7-step Builder UI, the approval card) and Phase 11 (`applications` list/patch/notes/timeline, Kanban) own remains untouched, as scoped.
