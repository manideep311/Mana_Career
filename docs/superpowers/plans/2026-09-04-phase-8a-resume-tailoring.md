...
| `backend/alembic/versions/0011_resume_tailoring.py` | Create | 3 tables, `updated_at` trigger on all 3 (`resume_chunks` mirrors `job_chunks` exactly, trigger included), `job_chunks`-style HNSW block for `resume_chunks` |
...
- [ ] **Step 2: `app/models/resume_version.py`** — three models per spec §5:
  ...
  - `ResumeChunk(Base, TimestampMixin)` → `resume_chunks`. **R-tsv (corrected from the spec):** the live `job_chunks` precedent (`app/models/job.py` `JobChunk`) HAS `TimestampMixin` + an `updated_at` trigger — it is not append-only-without-a-trigger as the spec assumed. Copy it exactly: `TimestampMixin`, `content_tsv: Mapped[str] = mapped_column(TSVECTOR, Computed("to_tsvector('english', content)", persisted=True))`, `embedding: Mapped[list[float] | None] = mapped_column(Vector(1024))`, and `resume_chunks` DOES get an `updated_at` trigger (`trg_resume_chunks_set_updated_at`) — same as `job_chunks`.
...
- [ ] **Step 4: `alembic/versions/0011_resume_tailoring.py`** — `revision="0011_resume_tailoring"`, `down_revision="0010_ai"`. `upgrade()`: `op.create_table("resume_versions", …)` + its 3 indexes + `CREATE TRIGGER trg_resume_versions_set_updated_at …` (copy the exact phrasing from `0010_ai.py`); `op.create_table("resume_chunks", …)` with `created_at`/`updated_at` columns, the `sa.Column("content_tsv", pg.TSVECTOR, sa.Computed("to_tsvector('english', content)", persisted=True))` + HNSW index (`postgresql_using="hnsw"`, `postgresql_with={"m": 16, "ef_construction": 64}`, `postgresql_ops={"embedding": "vector_cosine_ops"}` — copy `0007_jobs.py`'s `job_chunks` HNSW block verbatim) + GIN on `content_tsv` + the `(resume_version_id, chunk_index)` index + `CREATE TRIGGER trg_resume_chunks_set_updated_at …`; `op.create_table("resume_suggestions", …)` + 2 indexes + `CREATE TRIGGER trg_resume_suggestions_set_updated_at …`. **All three tables get the `updated_at` trigger this time** (unlike Phase 7a's `ai_actions`/`messages`, none of Phase 8's three tables are append-only logs). `downgrade()`: drop `resume_suggestions` → `resume_chunks` → `resume_versions`, with `DROP TRIGGER IF EXISTS …` before EACH of the three `drop_table`s.
...
**Interfaces:**
- Consumes: `AgentDeps` (typed `"AgentDeps"` under `TYPE_CHECKING` from `app.domain.agents.graph` — no `# type: ignore` needed, `graph.py` exists since Phase 7a); `GenerationService`; `tailor_resume` / `ClaimValidator` / `_collect_sources` (Task 3); `TailoringService` (Task 6); `ResumeService` (`get`, `list_`); `JobService` (`get`); `ResumeExtraction`; `app.domain.agents.blocks.{TextBlock, ResumeSuggestionBlock, dump_blocks}`. **`ProfileService` — verified real signatures (not the spec's assumed flat shape):**
  - `async def load_full(self, user_id) -> tuple[CareerProfile, dict[str, list[Any]]]` — returns `(profile, sections)` where `sections` keys are the sub-entity names (`experiences`/`education`/`projects`/`certifications`, per `SUBENTITY_MODELS`) each a `list` of that ORM model's rows.
  - `async def list_skills(self, user_id) -> list[tuple[ProfileSkill, Skill]]` — joined rows, not a flat list of labels.
- Produces:
...
- [ ] **Step 3: Implement.** `state.py` — extend the `AgentGoal` Literal and `NODE_ORDER`. `supervisor.py` — insert the `tailor_resume` branch. `resume_tailoring.py` / `claim_validator.py` per spec §4.2–§4.3, budget bump INLINE. `respond.py` — insert the `tailored_resume_version_id` branch as the first `if` in the block-building section (keep the rest untouched). `nodes/__init__.py` — add the two exports.
  - `_summarise_profile(profile: CareerProfile, sections: dict[str, list[Any]], skills: list[tuple[ProfileSkill, Skill]]) -> str` and `_summarise_job(job: Job) -> str` are private helpers in `resume_tailoring.py` (≤1.5k / ≤6k chars — truncate). Call site: `profile, sections = await ProfileService(deps.session).load_full(deps.user_id)`; `skills = await ProfileService(deps.session).list_skills(deps.user_id)`; `profile_summary = _summarise_profile(profile, sections, skills)`. `_summarise_profile` renders: `profile.location`/`preferred_roles`/`years_experience`/`seniority`/`career_goals` (whichever are non-null) + each `sections["experiences"]` row's `company`/`title`/`highlights`/`description` + each `skills` row's `Skill.label`. Field names on the sub-entity ORM rows mirror `app/models/profile.py` (`ProfileExperience.company/title/highlights/description`, etc.) — read that file if a field name is unclear rather than guessing.
...
**Status: COMPLETE** — 9 implementation tasks + verification, subagent-driven. 6 inline reviews + 3 subagent reviews (T5 migration, T8 graph wiring, T9 `/resumes` API) — all SPEC PASS / QUALITY APPROVED, 0 blocking findings. 6 thematic commits on `main`.

- **What changed:** `app/domain/generation/` (`GenerationService` — schema-validated LLM calls with a stable prompt hash + `GenerationMeta`); `app/domain/resume/tailoring.py` (deterministic `ClaimValidator` — token-overlap grounding check — and `tailor_resume()`, ≤2 reprompts on unsupported claims); `app/domain/documents/renderer.py` (`DocumentRenderer` — Markdown/HTML always, PDF/DOCX best-effort behind a `RenderUnavailable` + `doc_render_enabled` kill-switch); migration `0011_resume_tailoring` (`resume_versions`, `resume_chunks` [vector, unpopulated this phase], `resume_suggestions`); `app/domain/resume/version_service.py` (`TailoringService` CRUD + a deterministic field-level `diff()` between two `ResumeExtraction`s); two new LangGraph nodes (`resume_tailoring`, `claim_validator`) behind a new `tailor_resume` agent goal, wired supervisor→resume_tailoring→claim_validator→respond; `respond` gained a leading branch emitting a `resume_suggestion` block; `/resumes` gained `POST .../tailor` (202, starts an agent run), `GET .../versions`, `GET versions/{id}`, `GET versions/{id}/diff`, `GET versions/{id}/render` + the `llm` rate-limit bucket.
...
- **Files changed / new deps:** 37 files, +2478/-16, all under `backend/`. New: 3 domain modules (`generation`, `documents`, `resume/tailoring.py` + `version_service.py`), `models/resume_version.py`, `alembic/versions/0011_resume_tailoring.py`, 2 agent nodes, 12 test files. Modified (surgical): `core/config.py` (+`doc_render_enabled`), `core/rate_limit.py`, `agents/{graph,state}.py`, `agents/nodes/{__init__,respond,supervisor}.py`, `models/__init__.py`, `api/v1/resumes.py` + `schemas/resume.py`, `pyproject.toml`/`uv.lock` (+`markdown-it-py`, `+xhtml2pdf`, `+python-docx`).
...
- **Regression check:** Phases 0–7 suites green; alembic chain `…→0010→0011` linear, single head `0011_resume_tailoring`; `import-linter` 3 contracts kept (no new contract — `generation`/`documents` are `domain`-layer leaves); `/ai`, `/jobs`, `/matches`, `/eval` unchanged; the `understand_job`/`enrich_job` graph paths unchanged (supervisor gains one branch, respond gains one leading branch, both additive); no libpq needed for lint/type/collect (verified — `psycopg` never enters `sys.modules`).
- **Baseline:** backend collect 350 → 378 (+28); mypy 134 → 142 source files; import contracts 3 → 3.
- **As-built rulings (recorded in the SDD ledger):** **R-tsv** — the spec assumed `resume_chunks` was append-only with no trigger; the live `job_chunks` precedent it was meant to copy actually HAS `TimestampMixin` + an `updated_at` trigger, so `resume_chunks` was corrected to match exactly (caught in pre-flight, before any dispatch). `ProfileService.load_full`/`list_skills`'s real return shapes (`tuple[CareerProfile, dict[str,list]]` / joined `(ProfileSkill, Skill)` rows) were verified and substituted for the spec's assumed flat shape, also in pre-flight. Diff algorithm (`_diff_list` set-based added/removed/reordered, reused for both top-level `skills` and every sub-entity's `highlights`/`tech`) was worked out and traced against all 5 given tests before dispatch, since the plan text underspecified it — the implementer generalized it further into a data-driven `_SUB_SECTIONS` table. `to_pdf`/`to_docx` render REAL bytes on the dev box (not `RenderUnavailable`) — only `xhtml2pdf` needed a mypy `ignore_missing_imports` override. `ConflictError` (409, pre-existing in `app/core/errors.py`) is used for the render-unavailable case, superseding the plan's "bare Response" fallback once it was found. Two closeout fixes: the diff route's `against` UUID parse is now guarded (raises a clean `ValidationAppError` instead of an unhandled 500 on a malformed query param); `graph.py`'s module docstring updated from "eight/six nodes" to "ten/eight".
- **Deviations / out of scope:** cover letter / email / approval / send — Phases 9–10; `resume_chunks` populated + retrieval — Phase 12; persisting rendered docs to `FileStore` — later; `ResumeVersionBlock` in the registry — 8b (8a reuses the `resume_suggestion` stub); `manual_edit` in-place editing — later; generation LLM-judge eval — Phase 9.
- **Not verified here:** real tailoring quality (fake LLM in all tests); real PDF/DOCX visual fidelity; the diff on large/deeply-nested résumés; concurrent tailor runs on one résumé. The DB-backed suites (migration model roundtrip, `TailoringService`+diff over real data, the full `run_agent` tailoring trace, the `/resumes` version/diff/render API) run only in CI (no local Postgres).
...
- §5 models + migration `0011` (3 tables, `updated_at` trigger on all 3, `job_chunks`-style `resume_chunks` incl. its trigger — corrected from the spec via R-tsv) → Task 5. ✓
