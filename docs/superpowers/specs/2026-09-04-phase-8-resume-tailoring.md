...
`id` · `resume_version_id` FK CASCADE not null · `owner_id` uuid not null · `chunk_index` int not null · `section` String(40) not null · `ref_id` String(80) null · `content` Text not null · `token_count` int not null server_default `0` · `embed_model` String(80) not null server_default `text("'fake-embed-1'")` · `embed_dim` int not null server_default `text("1024")` · `embedding` `Vector(1024)` null · `content_tsv` `TSVECTOR` `Computed("to_tsvector('english', content)", persisted=True)` · created_at only (no `TimestampMixin` — append-only chunk). Indexes: HNSW on `embedding` (`vector_cosine_ops`, `m=16, ef_construction=64` — copy `job_chunks`), GIN on `content_tsv`, `ix_resume_chunks_version` (`resume_version_id`, `chunk_index`).
**Ruling R-tsv:** `content_tsv` follows the LIVE `job_chunks.content_tsv` precedent exactly — `mapped_column(TSVECTOR, Computed("to_tsvector('english', content)", persisted=True))` in the model and the same in the migration. `job_chunks` ships this generated column with green CI (`app/models/job.py:162`), so the summary's "no generated columns" caution does not apply to this proven pattern. Fallback if the migration CI run goes red: drop `Computed`, plain `TSVECTOR` nullable column, no index change — a one-line edit. Phase 8 writes NO rows to `resume_chunks`; the table exists so 8b / Phase 12 retrieval has it.
...
Migration: `revision="0011_resume_tailoring"`, `down_revision="0010_ai"`. Create order `resume_versions` → `resume_chunks` → `resume_suggestions`; downgrade reversed; `updated_at` triggers on `resume_versions` + `resume_suggestions` only (`resume_chunks` append-only, no trigger). Mirror `0010_ai.py` style for the two triggered tables; mirror `0007_jobs.py`'s `job_chunks` block for `resume_chunks` (the `Computed` tsv column + HNSW/GIN indexes). `Vector` import from `pgvector.sqlalchemy` (already a dep, used by `job_chunks` — `app/models/job.py:7`).
